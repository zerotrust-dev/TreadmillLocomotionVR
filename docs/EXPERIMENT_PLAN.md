...
- `wButtons` (raw bitmask — this is how we discover the sprint button)
...
how *sprint* arrives, which is the more important unknown.

## Success Criteria

Primary question: **how does the treadmill deliver the sprint/running signal?**
...
- Does a controller appear on any slot while the RealityRunner desktop app is
  ...
  emulates a **gamepad** and sprint is an XInput button we can replicate.
- If no pad is present but sprint still works in game, the app is emulating a
  **VR controller** (or synthesizing keyboard/mouse) and the sprint path is
  *not* XInput — which changes our output design for sprint.
- Which `wButtons` bit toggles when crossing the sprint threshold on the
...
2. Run `joy.cpl` with the app running. If a virtual pad is listed, open
   Properties and watch which button lights up when crossing the sprint
